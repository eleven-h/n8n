...
## 2.5 Chat Trigger（特殊触发器）

Chat Trigger 允许通过“聊天窗口”触发 Workflow，并将消息传递给下游 AI 节点。特性：
- 无参数设置，放到画布即可使用
- 每发送一次消息，触发一次完整的 Workflow 运行
- 缺省不具备跨次对话记忆（未接入 Memory 时）
- 可作为“输入器”使用，不一定必须直连 AI 节点
